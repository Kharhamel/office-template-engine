...
class OpenTBSPlugin extends TBSZip
{
    ...
    public function onInstall(): array
    {
        ...
        if (!isset($TBS->OtbsDeleteObsoleteChartData)) {
            $TBS->OtbsDeleteObsoleteChartData = true;
        }
    ...
    function TbsPicAdd(&$Value, PicVariable $PrmLst, &$Txt, &$Loc, $Prm)
    {
        ...
        if ($this->archive->fileGetIdxAdd($InternalPath)===false) {
    ...
    */
    ...
    function OpenXML_ChartSeriesFound(&$Txt, $SeriesNameOrNum, $OnlyBounds = false)
    {
        ...
        for ($i=1; $i<=2; $i++) {
            ...
            if ($p2===false) {
                return "Cached data not found for categories or values.";
            }
            $p2 = $p2 - 7;
            $res['point'.$i.'_p'] = $p1;
