...
{
    // Render flags
    public const TBS_NOTHING = 0;
    public const TBS_OUTPUT = 1;
    public const TBS_EXIT = 2;

    // Plug-ins actions
    public const TBS_INSTALL = -1;
    public const TBS_ISINSTALLED = -3;
    ...
    // Auto-installing plug-ins
        if (isset($_TBS_AutoInstallPlugIns)) {
            foreach ($_TBS_AutoInstallPlugIns as $pi) {
                $this->PlugIn(self::TBS_INSTALL, $pi);
            }
        }
    }
...
// Public methods
    ...
    public function Show($Render = false)
    {
        ...
        if ($this->meth_Misc_IsMainTpl()) {
            if (($Render & self::TBS_OUTPUT)==self::TBS_OUTPUT) {
                echo $this->Source;
            }
            if (($Render & self::TBS_EXIT)==self::TBS_EXIT) {
                exit;
            }
    ...
    public function PlugIn($Prm1, $Prm2 = 0)
    {
        ...
        if (is_numeric($Prm1)) {
            switch ($Prm1) {
                ...
                case self::TBS_ISINSTALLED: // Check if the plug-in is installed
